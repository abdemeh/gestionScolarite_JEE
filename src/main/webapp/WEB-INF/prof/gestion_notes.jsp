...
<%@ page import="springboot.modelesringboot.Resultat" %>
<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%
    /*
    // Session verification
    String userRole = (String) session.getAttribute("user");
    if (userRole == null || !"prof".equals(userRole)) {
...
<main>
    ...
    <div class="container">
        ...
        <% } %>
        <form action="<%= request.getContextPath() %>/modifierNotes" method="post">
        ...
        <table>
                ...
                <tbody>
                <% List<Resultat> notes = (List<Resultat>) request.getAttribute("resultats");
                    if (notes != null && !notes.isEmpty()) {
                        for (Resultat note : notes) { %>
                <tr>
                    ...
                    <td>
                        <input type="hidden" name="id_resultat" value="<%= note.getIdResultat() %>"/>
                        <input type="number" name="note_<%= note.getIdResultat() %>" value="<%= note.getNote() %>" step="0.01"/>
                    </td>
                </tr>
                <% }
                } else { %>
